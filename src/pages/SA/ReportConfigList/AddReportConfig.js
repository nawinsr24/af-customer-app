...
import { addReportConfig } from '../../../services/bo-service';
import shipmtReptFieldsArr from '../../../utils/shipmtReptFieldsArr';


function AddReportConfig({ modalOpen, handleModalClose }) {
            ...
            <form onSubmit={handleSubmit} >
                <Box sx={modalStyle}>
                    <Stack direction={'row'} alignItems={"center"} mb={2}>
                        <Typography sx={{ fontWeight: "bold", fontSize: 17, color: "rgba(0, 0, 0, 0.6)", mr: 1 }}>{t("sa.addReptConfig.addReptConfig")}</Typography>
                    </Stack>
                    <FillLabelTxtField name="configName" title={t("sa.addReptConfig.reptConfigName")} errMsg={formErrors.configName} {...txtFieldProps} />

                    <Stack direction={"row"} mt={3} gap={5} >
                        <Stack>
                            <Typography style={{ fontWeight: "bold", fontSize: 14 }} >{t("sa.addReptConfig.fieldUnselect")}</Typography>
                            <Box sx={boxStyle}>
                        ...
                        <Stack>
                            <Typography style={{ fontWeight: "bold", fontSize: 14, color: formErrors.selEleArr && "red" }} >{t("sa.addReptConfig.fieldSelect")}
                    ...
                    <Stack justifyContent={"flex-end"} direction={"row"} sx={{ mt: 2 }} width={"100%"}>
                        <Button variant="text" sx={{ height: 40, width: 120 }} onClick={handleModalClose}>{t("sa.addReptConfig.cancel")}</Button>
                        <Button variant="contained" type='submit' sx={{ height: 40, width: 150 }}>{t("sa.addReptConfig.save")}</Button>
                    </Stack>
                </Box>
            </form>
        </Modal>
