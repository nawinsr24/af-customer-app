...
import TrkOpReqTable from '../../../components/TrkOpReqTable';
...
function TrkOpReqListPage() {
  const { t } = useTranslation();
  const { time } = useParams();

  const ddArr = [
    { value: "name", label: t("bo.tpReqLstPg.dOperator"), },
    { value: "reqId", label: t("bo.tpReqLstPg.dReqId"), },
    { value: "mobile", label: t("bo.tpReqLstPg.dContact"), },
  ];
  ...
  function handleCust() {
    navigate(route.boCustReqList + time);
  }

  async function handleAddReq() {
    navigate(route.boAddTrkOpReq);
  }
  ...
  return (
    <Box height={"100%"} width={"100%"}>
      <Stack direction={'row'} justifyContent='space-between' mb={1.5} >
        <Typography variant='h5' sx={{ fontWeight: "bold" }}>{time === "current" ? t("bo.tpReqLstPg.reqLstScrn") : t("bo.tpReqLstPg.reqHis")}</Typography>
        <Stack mr={5}>
          <Stack direction={'row'} mx={1} gap={2}>
            <ButtonBase component="div" onClick={handleCust}>  <Typography sx={{ fontWeight: "600", }}>{t("bo.tpReqLstPg.cust")}</Typography></ButtonBase>
            <ButtonBase component="div"> <Typography sx={{ fontWeight: "600", color: "primary.main" }}>{t("bo.tpReqLstPg.trkOp")}</Typography></ButtonBase>
          </Stack>
          <Box sx={{ width: 228, backgroundColor: "#E3E3E3", borderRadius: 10, height: 5, mt: 0.5 }}>
            <Stack direction={'row'} height={4}>
              <Box sx={{ width: 88, borderRadius: 10 }} />
              <Box sx={{ width: 140, borderRadius: 10, backgroundColor: "primary.main" }} />
            </Stack>
          </Box>
        </Stack>
      </Stack>
      ...
      <BgBox height={"95%"} px={4} py={1}>
        ...
        <Stack direction={'row'} justifyContent="space-between" alignItems={"center"} mb={1.2} mt={0.4} px={2}>
          ...
          <Stack direction={'row'} justifyContent="space-between" alignItems={"center"}>
            <Box mb={0.5}><SearchTxtField variant={"outlined"} searchKeyWord={searchWord} onSearch={onSearch} onBtnClick={onSearchBtnClick} placeholder={t("bo.tpReqLstPg.search") + ` ${getSrchTypeTxt()} .....`} /></Box>
            ...
            <Button variant='contained' sx={{ height: 36, mb: 0.5 }} startIcon={<Add />} color='primary' onClick={handleAddReq}>{t("bo.tpReqLstPg.addReq")}</Button>
          </Stack>
        </Stack>


        {view === 'list' ? <TrkOpReqTable trkOpReqRow={trkOpReqRow?.data} /> :
          <ScrollBox height={"84%"}>
            <Grid container spacing={1}
              ...
              alignItems="center"
            >
              {trkOpReqRow?.data.map((i) => <Grid item xs="auto" key={i.id}> <TrkOpReqCard i={i} /> </Grid>)}
            </Grid>
          </ScrollBox>
        }
...
export default TrkOpReqListPage;